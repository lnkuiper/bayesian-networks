...
\section{Changes}
We firstly looked at the unconditioned dependencies and tried to add them to our
model. After that, we did the same with the conditioned variables. A changelog
...
dependency between the contact type and the month. After a quick look, this
...
changed only once in time. The first two years of the data collection was done
via landline, while the last three years all calls were made via a mobile phone.
We finally decided to delete the contact type alltogether. 

\subsection{Latent variables}
Education and duration were dependent on eachother, conditioned on five other
...
variables were removed. This latent variable later became wealth, where it als
...
made us delete the duration alltogether. 

A few other arrows and vertexes got deleted because they were not meaningfull:

\begin{itemize}
    \item the vertex day\_of\_week
    \item the vertex campaign
\end{itemize}
